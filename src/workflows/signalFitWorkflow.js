  ...
  async run(event, step) {
    ...
    try {
      ...
      const decision = await step.do("decide", async () => {
        // The fit rows only carry serializable data (no function refs),
        // so re-look-up the actual strategy object from the static registry.
      ...
      // Keep this best-effort and independent from the failed step: the
      // request must not remain in `fitting`, and the user needs an answer.
      try {
        await step.do("notify-unexpected-error", async () => {
          try {
            await db.updateSignalRequestStatus(this.env, requestId, "failed");
          } catch (statusErr) {
            console.error("Could not mark signal request failed", statusErr);
          }
          await sendMessage(
            this.env,
            chatId,
            `⚠️ یه خطای غیرمنتظره حین تحلیل ${escapeHtml(symbol)} پیش اومد. دوباره امتحان کن.`
          );
        });
      } catch (notifyErr) {
        console.error("Could not notify signal fit failure", notifyErr);
      }

      return { outcome: "error", error: errorText };
    }
  }
}
